...
function b32 odin_parse_decl(Code_Index_File *index, Generic_Parse_State *state, Code_Index_Nest *parent, Token *ident)
{
    ...
    {
        // print_message(state->app, SCu8(" Colon Colon Again\n"));
        generic_parse_inc(state);
        generic_parse_skip_soft_tokens(index, state);
        peek = token_it_read(&state->it);
        if (peek == 0)
            return false;
        // print_message(state->app, SCu8("Trying Odin Index Decl\n"));
...
function b32 odin_try_index(Code_Index_File *index, Generic_Parse_State *state, Token *token)
{
